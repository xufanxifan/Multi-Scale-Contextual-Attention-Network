...
__C.ASSETS_PATH = '/home/user/data/hkscapesplus_gai'
# __C.ASSETS_PATH = '/home/user/data/facadewhu/facadewhu/facadewhu_resize'
# __C.ASSETS_PATH = '/home/user/data/cityscape/assets'
...
#hkscapes Dir Location
__C.DATASET.HKSCAPES_DIR = \
  os.path.join(__C.ASSETS_PATH, 'data')
__C.DATASET.HKSCAPES_CUSTOMCOARSE = \
...
#FACADEWHU Dir Location
__C.DATASET.FACADEWHU_DIR = \
  os.path.join(__C.ASSETS_PATH, 'data')
__C.DATASET.FACADEWHU_CUSTOMCOARSE = \
  os.path.join(__C.ASSETS_PATH, 'data/autolabelled')
__C.DATASET.CENTROID_ROOT = \
  os.path.join(__C.ASSETS_PATH, 'uniform_centroids')
#SDC Augmented Cityscapes Dir Location
...
__C.DATASET.HKSCAPES_SPLITS = 3
__C.DATASET.FACADEWHU_SPLITS = 3
...
__C.DATASET.MASK_OUT_HKSCAPES = False
__C.DATASET.MASK_OUT_FACADEWHU = False
...
def assert_and_infer_cfg(args, make_immutable=True, train_mode=True):
    ...
    if args.dump_assets and args.dataset == 'cityscapes':
        ...
        logx.msg('*' * 70)
        cfg.DATASET.CV = 3
    elif args.dump_assets and args.dataset == 'hkscapes':
        # A hacky way to force that when we dump hkscapes
        logx.msg('*' * 70)
        logx.msg(f'ALERT: forcing cv=3 to allow all images to be evaluated')
        logx.msg('*' * 70)
        cfg.DATASET.CV = 3
    elif args.dump_assets and args.dataset == 'facadewhu':
        # A hacky way to force that when we dump facadewhu
        logx.msg('*' * 70)
        logx.msg(f'ALERT: forcing cv=3 to allow all images to be evaluated')
        logx.msg('*' * 70)
        cfg.DATASET.CV = 3
    ...
    if args.set_hkscapes_root is not None:
        # '/data/cs_imgs_cv0'
        # '/data/cs_imgs_cv2'
        __C.DATASET.HKSCAPES_DIR = args.set_hkscapes_root

    if args.set_facadewhu_root is not None:
        # '/data/cs_imgs_cv0'
        # '/data/cs_imgs_cv2'
        __C.DATASET.FACADEWHU_DIR = args.set_facadewhu_root
